...
namespace CraftNetTools
{
    class AppUpdates
    {
        public static void Check()
        {
            ...
            string name = "STREDIT for OldMS";
            int version = int.Parse(Application.ProductVersion.Replace(".", ""));

            string derp = string.Format("http://www.craftnet.nl/app_updates/updates.php?appname={0}&appver={1}", name, version);

            try
            {
                HttpWebRequest req = WebRequest.Create(derp) as HttpWebRequest;
                //req.Timeout = 2000;
                req.Proxy = null;
